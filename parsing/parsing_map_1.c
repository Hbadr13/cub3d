...
void	check_map_characters(char **map, int i, int j)
{
	while (map[i][j])
	{
		...
		{
			if (!if_0_or_1_or_player(map[i][j + 1]))
				msg_error(1);
			if (!if_0_or_1_or_player(map[i][j - 1]))
				msg_error(1);
			if (!if_0_or_1_or_player(map[i + 1][j - 1]))
				msg_error(1);
			if (!if_0_or_1_or_player(map[i + 1][j + 1]))
				msg_error(1);
			if (!if_0_or_1_or_player(map[i + 1][j]))
				msg_error(1);
			if (!if_0_or_1_or_player(map[i - 1][j + 1]))
				msg_error(1);
			if (!if_0_or_1_or_player(map[i - 1][j - 1]))
...
void	map_characters(char **map, int i, int j)
{
	if (map[i][j] != '0' && map[i][j] != '1'
		&& map[i][j] != ' ' && !if_player(map[i][j]) && map[i][0] != '\n')
		msg_error(5);
	if (i > 1)
	{
		if (map[i][j] == 'D')
